...
fit.svm <- ksvm(training$target~.,data=training,type="C-bsvc",
                  kernel=rbf,C=10,prob.model=TRUE)
fit.svm
## get fitted values
fitted(fit.svm)

## Test on the training set with class/probabilities as output
predsvm.class<-predict(fit.svm, testing[,-21])
predsvm.class

predsvm.prob<-predict(fit.svm, testing[,-21],type="probabilities")
predsvm.prob

## Check results
table(predsvm.class,testing[,21])

##calvulate indexes 
predType <-c()
for (i in 1:length(predsvm.class))
{
  if (predsvm.class[i]=="good")
  {
    predType[i]<-1
  }
  if (predsvm.class[i]=="bad")
  {
    predType[i]<-0
  }
}
